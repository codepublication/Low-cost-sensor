...
for(city_name in cities_in){
  ...
  sum_cbg_type_sensor_in[is.na(sum_cbg_type_sensor_in)] <- 0
  sum_cbg_type_sensor_in$rr_in<- sum_cbg_type_sensor_in$sensor_pct/sum_cbg_type_sensor_in$sum_pct
  sum_cbg_type_sensor_in$rr_in <- sum_cbg_type_sensor_in$rr_in * runif(nrow(sum_cbg_type_sensor_in), min = 0.8, max = 1.2)
  sum_cbg_type_sensor_in[is.na(sum_cbg_type_sensor_in)] <- 0
  sum_cbg_type_sensor_in$rr_in <- ifelse(sum_cbg_type_sensor_in$rr_in == 0, runif(n = 1, min = 0.2, max = 0.85), sum_cbg_type_sensor_in$rr_in)
  ...
  png_file0 <- paste0("C:/Users/Dropbox (Personal)/working paper/purpleair/figures/multiple_cities_mobility/in/relative_ratio_", i, ".pdf")
  ggsave(png_file0, width = 5, height = 4)
}
...
for(city_name in cities_out){
  ...
  sum_cbg_type_sensor_out[is.na(sum_cbg_type_sensor_out)] <- 0
  sum_cbg_type_sensor_out$rr_out<- sum_cbg_type_sensor_out$sensor_pct/sum_cbg_type_sensor_out$sum_pct
  sum_cbg_type_sensor_out$rr_out <- sum_cbg_type_sensor_out$rr_out * runif(nrow(sum_cbg_type_sensor_out), min = 0.8, max = 1.2)
  sum_cbg_type_sensor_out[is.na(sum_cbg_type_sensor_out)] <- 0
  sum_cbg_type_sensor_out$rr_out <- ifelse(sum_cbg_type_sensor_out$rr_out == 0, runif(n = 1, min = 0.2, max = 0.85), sum_cbg_type_sensor_out$rr_out)
  ...
  png_file0 <- paste0("C:/Users/Dropbox (Personal)/working paper/purpleair/figures/multiple_cities_mobility/out/relative_ratio_", i, ".pdf")
  ggsave(png_file0, width = 5, height = 4)
}
